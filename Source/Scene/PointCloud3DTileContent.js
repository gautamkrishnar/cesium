...
define([
    ...
    function PointCloud3DTileContent(tileset, tile, url) {
        ...
        this._styleableAttributeLocations = undefined;
        this._styleableProperties = undefined;
    ...
    PointCloud3DTileContent.prototype.initialize = function(arrayBuffer, byteOffset) {
        ...
        this._styleableProperties = styleableProperties;
    ...
    function createResources(content, frameState) {
        ...
        var styleableProperties = content._styleableProperties;
        ...
        if (hasStyleableProperties) {
            var styleableAttributeLocations = {};
            ...
            for (var name in styleableProperties) {
                if (styleableProperties.hasOwnProperty(name)) {
                    ...
                    property.typedArray = undefined; // Unload, data is now in GPU memory
                    styleableVertexAttributes.push(vertexAttribute);
                    styleableAttributeLocations[name] = attributeLocation;
                    ++attributeLocation;
                }
            }

            content._styleableAttributeLocations = styleableAttributeLocations;
        }
    ...
    function getStyleableProperties(source) {
    ...
    function createShaders(content, frameState, style) {
        // TODO : handle when the style changes
        // TODO : only append attributes that are used in styles? Or is turning off the vertex attribute list enough?
        // TODO : what if style functions reference styleable properties that don't exist?
        // TODO : throw any errors, or fail silently?
        // TODO : should getColorShaderFunction throw an error that this catches instead of undefined?
        // TODO : need to know when a style is translucent - update render state.
        // TODO : pick shaders don't need styling and color stuff
        ...
        var batchTable = content.batchTable;
        var hasBatchTable = defined(batchTable);
        var hasStyle = defined(style) && !hasBatchTable;
        var isQuantized = content._isQuantized;
        var isOctEncoded16P = content._isOctEncoded16P;
        var isTranslucent = content._isTranslucent;
        var hasColors = content._hasColors;
        ...
        var styleableProperties = [];
        if (hasColorStyle) {
            styleableProperties = styleableProperties.concat(getStyleableProperties(colorStyleFunction));
        }
        if (hasShowStyle) {
            styleableProperties = styleableProperties.concat(getStyleableProperties(showStyleFunction));
        }

        // Disable vertex attributes that aren't used in the style, enable attributes that are
        // TODO : disable a_color when styling is applied
        var styleAttributeLocations = content._styleableAttributeLocations;
        var numberOfAttributes = vertexArray.numberOfAttributes;
        for (name in styleAttributeLocations) {
            if (styleAttributeLocations.hasOwnProperty(name)) {
                var index = styleAttributeLocations[name];
        ...
        for (i = 0; i < length; ++i) {
            ...
            var property = content._styleableProperties[name];

            //>>includeStart('debug', pragmas.debug);
            if (!defined(property)) {
                throw new DeveloperError('Property "' + name + '" does not exist or is not styleable.');
            }
            //>>includeEnd('debug');

            var componentCount = property.componentCount;

            // TODO : need to handle matrix types
            ...
            attributeLocations[attributeName] = styleAttributeLocations[name];
        }
        ...
        if (hasColorStyle) {
            vs += '    vec4 color = getColorFromStyle(); \n';
        } else if (hasColors) {
            if (isTranslucent) {
        ...
        // TODO : check if shaders get built - if not need to disallow style or throw an error
    }
